...
function addDecache(image) {
  image.decache = function() {
    var parsedImageId = cornerstoneWADOImageLoader.wadouri.parseImageId(
      image.imageId
    );
...
function getPixelData(dataSet, frameIndex) {
  ...
  if (pixelDataElement.encapsulatedPixelData) {
    return cornerstoneWADOImageLoader.wadouri.getEncapsulatedImageFrame(
      dataSet,
      frameIndex
    );
  } else {
    return cornerstoneWADOImageLoader.wadouri.getUncompressedImageFrame(
      dataSet,
      frameIndex
    );
  }
...
function loadDataSetFromPromise(
  xhrRequestPromise,
  imageId,
  frame,
  sharedCacheKey,
  ...
  var deferred = new Promise(function(resolve, reject) {
    xhrRequestPromise.then(function(dataSet /*, xhr*/) {
      ...
      var imagePromise = cornerstoneWADOImageLoader.createImage(
        imageId,
        pixelData,
        transferSyntax,
        options
...
function loadImage(imageId, options) {
  ...
  if (
    cornerstoneWADOImageLoader.wadouri.dataSetCacheManager.isLoaded(
      parsedImageId.url
    )
  ) {
    return loadDataSetFromPromise(
...
function convertPALETTECOLOR(imageFrame, rgbaBuffer) {
  ...
  for (var i = 0; i < numPixels; ++i) {
    ...
    else if (value > start + len - 1) value = len - 1;
    else value = value - start;
...
function convertRGB(imageFrame, rgbaBuffer) {
  if (imageFrame.planarConfiguration === 0) {
    cornerstoneWADOImageLoader.convertRGBColorByPixel(
      imageFrame.pixelData,
      rgbaBuffer
    );
  } else {
    cornerstoneWADOImageLoader.convertRGBColorByPlane(
      imageFrame.pixelData,
      rgbaBuffer
    );
  }
...
function convertYBRFull(imageFrame, rgbaBuffer) {
  if (imageFrame.planarConfiguration === 0) {
    cornerstoneWADOImageLoader.convertYBRFullByPixel(
      imageFrame.pixelData,
      rgbaBuffer
    );
  } else {
    cornerstoneWADOImageLoader.convertYBRFullByPlane(
      imageFrame.pixelData,
      rgbaBuffer
    );
  }
...
function convertColorSpace(imageFrame, imageData) {
  ...
  if (imageFrame.photometricInterpretation === 'RGB') {
    ...
    throw 'no color space conversion for photometric interpretation ' +
...
function createImage(imageId, pixelData, transferSyntax, options) {
  var deferred = new Promise(function(resolve, reject) {
    ...
    decodePromise.then(function(imageFrame) {
      ...
      var imagePlaneModule = cornerstone.metaData.get(
        'imagePlaneModule',
        imageId
      );
      var voiLutModule = cornerstone.metaData.get('voiLutModule', imageId);
      var modalityLutModule = cornerstone.metaData.get(
        'modalityLutModule',
        imageId
      );
      var sopCommonModule = cornerstone.metaData.get(
        'sopCommonModule',
        imageId
      );
...
function decodeImageFrame(
  ...
  // JPEG Baseline lossy process 1 (8 bit)
    ...
    if (imageFrame.bitsAllocated === 8) {
      return cornerstoneWADOImageLoader.decodeJPEGBaseline8Bit(
        imageFrame,
        canvas
      );
    } else {
    ...
    r.onload = function() {
      ...
      if (r.readAsBinaryString === undefined) {
        img.src =
          ...
          window.btoa(arrayBufferToString(r.result));
      } else {
...
cornerstoneWADOImageLoader.wadors.getPixelData = function(
  uri,
  imageId,
  mediaType
) {
  ...
  var deferred = new Promise(function(resolve, reject) {
    var loadPromise = cornerstoneWADOImageLoader.internal.xhrRequest(
      uri,
      imageId,
      headers
    );
    ...
    loadPromise.then(function(imageFrameAsArrayBuffer /*, xhr*/) {
      ...
      var tokenIndex = cornerstoneWADOImageLoader.wadors.findIndexOfString(
        response,
        '\n\r\n'
      );
      if (tokenIndex === -1) {
      ...
      var endIndex = cornerstoneWADOImageLoader.wadors.findIndexOfString(
        response,
        boundary,
        offset
      );
  ...
  var deferred = new Promise(function(resolve, reject) {
    ...
    cornerstoneWADOImageLoader.wadors
      ...
      .then(function(result) {
        var transferSyntax = getTransferSyntaxForContentType(
          result.contentType
        );
...
function getNumberString(element, index, defaultValue) {
  var value = cornerstoneWADOImageLoader.wadors.getValue(
    element,
    index,
    defaultValue
  );
...
function getEncodedImageFrame(dataSet, frame) {
  ...
  if (!dataSet.elements.x7fe00010.basicOffsetTable.length) {
    if (framesAreFragmented(dataSet)) {
      var basicOffsetTable = dicomParser.createJPEGBasicOffsetTable(
        dataSet,
...
function metaDataProvider(type, imageId) {
  ...
  var dataSet = cornerstoneWADOImageLoader.wadouri.dataSetCacheManager.get(
    parsedImageId.url
  );
  ...
  if (type === 'modalityLutModule') {
    return {
      ...
      modalityLUTSequence: cornerstoneWADOImageLoader.wadouri.getLUTs(
  ...
  if (type === 'voiLutModule') {
    var modalityLUTOutputPixelRepresentation = cornerstoneWADOImageLoader.wadouri.getModalityLUTOutputPixelRepresentation(
      dataSet
    );
    return {
      windowCenter: getNumberValues(dataSet, 'x00281050', 1),
      windowWidth: getNumberValues(dataSet, 'x00281051', 1),
      voiLUTSequence: cornerstoneWADOImageLoader.wadouri.getLUTs(
...
function handleMessageFromWorker(msg) {
  if (msg.data.taskType === 'initialize') {
    ...
    var end = new Date().getTime();
    statistics.totalTaskTimeInMS +=
      end - webWorkers[msg.data.workerIndex].task.start;
